...
const LightPillar: React.FC<LightPillarProps> = ({
    ...
    `;
      ...
      void main() {
        ...
        float alpha = max(color.r, max(color.g, color.b)) * uIntensity;
        float edgeMask = smoothstep(0.0, 0.1, vUv.x) * smoothstep(1.0, 0.9, vUv.x) *
                 smoothstep(0.0, 0.1, vUv.y) * smoothstep(1.0, 0.9, vUv.y);
        
        gl_FragColor = vec4(color * uIntensity, alpha * edgeMask);
      }
    `;
    ...
    return (
        <div ref={containerRef} className={`w-full h-full absolute top-0 left-0 ${className}`}
             style={{mixBlendMode}}/>
    );
};
